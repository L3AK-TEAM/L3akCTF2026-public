SBOX = [
    ,
]

RCON = [0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40]

def _xor_words(a, b):
    return [x ^ y for x, y in zip(a, b)]

def _sub_word(word):
    return [SBOX[x] for x in word]

def _rot_word(word):
    return word[1:] + word[:1]

def _xtime(x):
    x <<= 1
    return (x ^ 0x11b) & 0xff if x & 0x100 else x

def _mix_single_column(col):
    t = col[0] ^ col[1] ^ col[2] ^ col[3]
    u = col[0]
    return [
        col[0] ^ t ^ _xtime(col[0] ^ col[1]),
        col[1] ^ t ^ _xtime(col[1] ^ col[2]),
        col[2] ^ t ^ _xtime(col[2] ^ col[3]),
        col[3] ^ t ^ _xtime(col[3] ^ u),
    ]

def _add_round_key(state, round_key):
    return [x ^ y for x, y in zip(state, round_key)]

def _sub_bytes(state):
    return [SBOX[x] for x in state]

def _shift_rows(state):
    out = state[:]
    for row in range(4):
        values = [state[row + 4 * col] for col in range(4)]
        values = values[row:] + values[:row]
        for col in range(4):
            out[row + 4 * col] = values[col]
    return out

def _mix_columns(state):
    out = state[:]
    for col in range(4):
        mixed = _mix_single_column(state[4 * col : 4 * col + 4])
        out[4 * col : 4 * col + 4] = mixed
    return out

def expand_key(master_key, rounds):

    assert len(master_key) == 24

    needed_words = 4 * (rounds + 1)
    words = [list(master_key[i : i + 4]) for i in range(0, len(master_key), 4)]

    i = 6
    while len(words) < needed_words:
        temp = words[-1][:]
        if i % 6 == 0:
            temp = _sub_word(_rot_word(temp))
            temp[0] ^= RCON[i // 6]
        words.append(_xor_words(words[-6], temp))
        i += 1

    return [bytes(sum(words[4 * r : 4 * r + 4], [])) for r in range(rounds + 1)]

def lock(plaintext, master_key, rounds):
    
    assert len(plaintext) == 16

    round_keys = expand_key(master_key, rounds=rounds)
    state = _add_round_key(list(plaintext), round_keys[0])

    for r in range(1, rounds):
        state = _sub_bytes(state)
        state = _shift_rows(state)
        state = _mix_columns(state)
        state = _add_round_key(state, round_keys[r])

    state = _sub_bytes(state)
    state = _shift_rows(state)
    state = _add_round_key(state, round_keys[rounds])
    return bytes(state)
